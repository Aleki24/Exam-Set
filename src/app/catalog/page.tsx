...
/**
 * THE CATALOGUE — search and filters over the whole stock.
 *
 * This was `/` for as long as the product was only an exam-paper shop, and the
 * argument for that was sound: the first thing anyone saw was the papers they
 * could buy, filtered the way a teacher actually shops (level, then exam type).
 *
 * It moved here when the shop became a library. A visitor who already knows
 * what they want still lands on exactly this page, one click from anywhere —
 * but somebody arriving cold now gets `/` instead, which can explain what the
 * platform is before asking them to operate a filter rail. Nothing about this
 * page changed in the move; it only stopped being the only door.
 */
export default function CatalogPage() {
    // `useSearchParams` opts the tree into client-side rendering, and Next
    // requires the boundary to be explicit or the build fails on this route.
    return (
 ...
 *
 * It was not, and that quietly broke every deep link into the shop: the kind
 * tiles on `/learn/[level]` have always pointed at `?level=…&kind=…`, the
 * sitemap has been advertising a `?level=` URL per level, and the page read
 * none of it — so all of them landed on the same unfiltered grid. Seeding from
 * the URL and writing back to it fixes the links that already exist, and is
 * what lets a category page be a real address rather than a click path.
 */
function Catalog() {
    ...
    const [filters, setFilters] = useState<PaperFilters>(() => filtersFromParams(searchParams));
    ...
    /**
     * Mirror the filters back into the address bar.
     *
     * `replace`, not `push`: ticking four filter boxes should not cost four
     * presses of the back button. `scroll: false` because the results are
     * already in view and yanking to the top on every chip is disorienting.
     *
     * The first run is skipped — the filters were seeded *from* the URL, so
     * rewriting it immediately would only strip the `limit`/`offset` a shared
     * link might carry.
     */
    ...
    const kindDef = filters.kind ? RESOURCE_KIND_BY_SLUG[filters.kind] : undefined;
                    ...
                    <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
                        <div className="min-w-0">
                            ...
                            <h1 className="display-2">
                                {activeSummary.length > 0 ? activeSummary.join(' · ') : 'Every resource'}
                            </h1>
                            {/* `description` on a kind is written "for the person
                                choosing", which is exactly this slot. */}
                            {kindDef ? (
                                ...
                                !filters.search && (
                                    <p className="lead mt-3 max-w-md text-sm sm:text-[15px]">
                                        Papers, marking schemes, schemes of work, lesson plans and notes —
                                        ready to print.
                                    </p>
                                )
                            )}
...
function AppliedFilters({
    ...
    if (filters.kind) {
        applied.push({
            label: RESOURCE_KIND_BY_SLUG[filters.kind]?.plural ?? filters.kind,
...
/**
 * The URL, read into filters.
 *
 * Unknown values are dropped rather than passed through: a hand-edited
 * `?sort=cheapest` should fall back to the default ordering, not send the API
 * a sort it has to decide what to do with.
 */
function filtersFromParams(params: URLSearchParams | ReadonlyURLSearchParams): PaperFilters {
...
/** Filters, written back out. The inverse of `filtersFromParams`. */
function paramsFromFilters(filters: PaperFilters): URLSearchParams {
    const params = new URLSearchParams();
    if (filters.level) params.set('level', filters.level);
    if (filters.grade) params.set('grade', filters.grade);
    if (filters.subject) params.set('subject', filters.subject);
...
/** What the Suspense boundary shows while the client picks up the URL. */
function CatalogFallback() {
    return (
        <div className="min-h-screen bg-background">
            <TopNav />
            <div className="shell-width py-10">
