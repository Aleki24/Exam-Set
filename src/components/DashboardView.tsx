...
interface DashboardViewProps {
    ...
    recentExams: any[]; // Placeholder type for now
}

const DashboardView: React.FC<DashboardViewProps> = ({ onNavigate, onMagicGenerate, recentExams }) => {
    return (
        <div className="p-8 max-w-7xl mx-auto">
            ...
            <div className="bg-card p-6 rounded-3xl border border-border shadow-sm mb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div
                        ...
                        className="bg-secondary p-4 rounded-xl border border-border hover:bg-accent transition-all cursor-pointer group"
                    >
            ...
            <div className="bg-card rounded-[2rem] border border-border shadow-sm p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="p-2 bg-secondary text-primary rounded-lg transition-colors">
                ...
                <div className="grid grid-cols-12 gap-4 text-[10px] font-black uppercase text-muted-foreground border-b border-border pb-3 mb-2 px-2">
                    ...
                    <div className="hidden md:block col-span-2">Size/Marks</div>
                    <div className="col-span-6 md:col-span-3 text-right pr-4">Modified</div>
                </div>

                {/* List Items */}
                <div className="space-y-1">
                    {recentExams.length > 0 ? recentExams.map((exam, i) => (
                        <div key={i} className="grid grid-cols-12 gap-4 items-center p-3 hover:bg-secondary rounded-xl transition-colors group cursor-pointer border border-transparent hover:border-border">
                            {/* Name Col */}
                            <div className="col-span-6 md:col-span-5 flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <p className="text-sm font-bold text-foreground truncate group-hover:text-primary transition-colors">{exam.metadata.title}</p>
                                    <p className="md:hidden text-[10px] text-muted-foreground">{exam.metadata.subject}</p>
                                </div>
                            </div>
                            ...
                            <div className="col-span-6 md:col-span-3 flex items-center justify-end gap-4">
                                ...
                                <button className="p-1 hover:bg-secondary rounded-lg text-muted-foreground transition-colors opacity-0 group-hover:opacity-100">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z" /></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="text-center py-10 opacity-50">
                            <p className="text-sm text-muted-foreground">No exams generated recently.</p>
                        </div>
                    )}
                </div>
            </div>
